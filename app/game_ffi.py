...
_libpath_tmp = _here / 'core' / _lib_name()
_libpath = str(_libpath_tmp.resolve())
...
class GameFFI:
    ...
    def init(self, rows, cols, bomb_count, difficulty='custom'):
        ...
        if self.h:
            self.free()
        self.h = self.lib.game_init(ctypes.c_int(rows), ctypes.c_int(cols), ctypes.c_int(bomb_count))
        return self.h

    def left_click(self, r, c):
        if not self.h:
            return -1
        return self.lib.game_left_click(self.h, int(r), int(c))

    def right_click(self, r, c):
        if not self.h:
            return -1
        return self.lib.game_right_click(self.h, int(r), int(c))

    def undo(self):
        if not self.h:
            return -1
        return self.lib.game_undo(self.h)

    def get_state(self):
        if not self.h:
            return '{}'
        buf = ctypes.create_string_buffer(65536)  # 64KB char buffer
        self.lib.game_get_state(self.h, buf, ctypes.c_size_t(len(buf)))
        return buf.value.decode('utf-8')

    def free(self):
        if not self.h:
            return
        self.lib.game_free(self.h)
        self.h = None
